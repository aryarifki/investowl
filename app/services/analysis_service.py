...
def sparkline_values(activity: pd.DataFrame, broker_code: str, end_ts: pd.Timestamp, days: int = 5) -> str:
    if activity.empty or not broker_code: return "-----"
...
def conviction_score(signal: object, foreign_5d: float | None, scan_df: pd.DataFrame, ticker: str, causality: dict | None = None) -> dict[str, object]:
    # Fix: Handle None/NaN safely like app.py
    p_value = None
    if causality:
        try:
            val = causality.get("min_p_value")
